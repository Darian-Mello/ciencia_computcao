...
void cadastrar_produto(produto *prods);
...
main(){
    ...
    do{
        ...
        if(opcao == 1){
            if(cont < 50){
                cadastrar_produto(&cadastros[cont]);
                cont++;
                leu = true;
            }
            else
            ...
            listar_produtos(&cadastros[0], cont);
        else if(opcao == 3)
            consultar_produto(&cadastros[0], cont);
        else
            vender_produto(&cadastros[0], cont);
...
void finaliza_venda(produto *prods) {
    ...
    if(quantidade <= prods->quantidade){
        ...
        cout << "\nA quntidade presente no estoque é de " << prods->quantidade << ", deseja vender uma quantidade de acordo com o estoque(1 - sim, 2 - não)? ";
        cin >> execute;
